...
This document explains the design in plain terms. [HLD.md](./docs/HLD.md) and
[LLD.md](./docs/LLD.md) hold the detail.

|                                    Map                                    |                              Selected friend                             |                                Friends list                               |
| :------------------------------------------------------------------------: | :------------------------------------------------------------------------: | :-------------------------------------------------------------------------: |
| ![Live map with identity-coloured pins and haloes](./docs/screenshots/map.png) | ![A tapped pin's name, highlight ring and detail sheet](./docs/screenshots/selected.png) | ![Friends list with matching avatar colours and all/any filter](./docs/screenshots/friends.png) |
...
Colour alone would still be undecodable, so it is never presented on its own:
the same mapping colours a person's row in the Friends list, next to their
name. The list is the key — no separate legend to keep in sync with it.
...
**Status.** Specified in [LLD §13](./docs/LLD.md) with the component contract, an
...
specified in [HLD §9](./docs/HLD.md) as design work rather than client rendering work.
